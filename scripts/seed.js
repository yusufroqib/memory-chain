...
async function main() {
  let memoryChainContract

  try {
    const contractAddresses = fs.readFileSync('./contracts/contractAddress.json', 'utf8')
    const { memoryChainContract: memoryChainAddress } = JSON.parse(contractAddresses)

    memoryChainContract = await ethers.getContractAt('MemoryChain', memoryChainAddress)

    // Process #1
    await Promise.all(
      generateGameData(dataCount).map(async (game) => {
        await createGame(memoryChainContract, game)
      })
    )
        ...
        .map(async (_, i) => {
          ...
          await Promise.all(
            invitations.map(async (player) => {
              await sendInvitation(memoryChainContract, player)
            })
          )
        })
    )
    ...
    // Process #3
    await getGames(memoryChainContract)
    await getInvitations(memoryChainContract, 1)
    await getMyInvitations(memoryChainContract)
  } catch (error) {
